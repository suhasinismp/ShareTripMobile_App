...
//       console.log('Confirmed Posted Data Response:', response?.data);
...
//       ? item?.accepted_user_profile || item?.posted_user_profile_pic || 'https://via.placeholder.com/150'
//       : item?.posted_user_profile_pic || item?.user_profile_pic || 'https://via.placeholder.com/150';

//     const userName = selectedFilterTwo === 'PostedTrips'
//       ? item?.accepted_user_name || item?.posted_user_name || 'User'
//       : item?.posted_user_name || item?.user_name || 'User';
...
const MyTrips = ({ route }) => {
  ...
  const fetchPostedGuyInProgressData = async () => {
    try {
      ...
      if (response?.error === false && Array.isArray(response.data)) {
        ...
        // console.log('Quoted trips found:', quotedArray.length);
        // Fetch driver details for quoted trips
        const quotedWithDrivers = await Promise.all(
          quotedArray.map(async (item) => {
            try {
              const driverResponse = await DriverData(item.post_booking_id, userToken);
              // console.log(`Driver response for ${item.post_booking_id}:`, driverResponse);

              if (!driverResponse?.error && Array.isArray(driverResponse?.data)) {
                const trackingDetails = driverResponse.data.map(driver => ({
                  id: driver.id,
                  post_id: item.post_booking_id, // helpful for accept/reject
                  user_id: driver.accepted_user_id,
                  user_name: driver.driver.name,
                  user_profile: driver.driver.profile_pic,
                  user_phone: driver.driver.phone_number,
                  vehicle_number: driver.driver.vehicle_registration,
                  vehicle_type: driver.driver.vehicle_type,
                  vehicle_name: driver.driver.vehicle_name,
                  vehicle_id: driver.vehicle_id,
                  confirm_status: driver.confirm_status,

                }));

                // console.log('Tracking details for post_booking_id:', item.post_booking_id, trackingDetails);
                return {
                  ...item,
                  trackingDetails
                };
        ...
        // console.log('Final Combined Trips Count:', finalCombined.length);
  ...
  // const renderItem = ({ item }) => {
  //   // console.log('abc', item?.User_profile || item?.user_profile_pic)
  //   // console.log({ selectedFilterTwo })
  //   if (
  //     selectedFilterOne === 'InProgress' &&
  //     selectedFilterTwo === 'PostedTrips' &&
  ...
  //         amount={item?.bookingTypeTariff_base_fare_rate}
  //         pickUpTime={item?.pick_up_time}
  ...
  //         postComments={item?.post_comments}
  //         postVoiceMessage={item?.post_voice_message}
  //         drivers={Array.isArray(item?.trackingDetails) ? item.trackingDetails : []}
  //         onCallPress={() => handleCall(item?.User_phone)}
  //         onMessagePress={() => handleChatPress(item)}
  //         onRefreshData={fetchUiData}
  //         userToken={userToken}
  ...
  //     : item?.User_profile || item?.user_profile_pic || 'https://via.placeholder.com/150';

  //   const userName = selectedFilterTwo === 'PostedTrips'
  //     ? item?.accepted_user_name || item?.User_name || 'User'
  //     : item?.User_name || item?.user_name || 'User';
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <PostCard
        ...
        pickUpTime={item?.pick_up_time || ''}
        fromDate={item?.trip_date || item?.from_date || ''}
        vehicleType={item?.Vehicle_type_name || item?.vehicle_type || ''
        }
        ...
        postComments={item?.post_comments || ''}
        postVoiceMessage={item?.post_voice_message || ''}
        baseFareRate={item?.bookingTypeTariff_base_fare_rate || item.booking_tarif_base_fare_rate || ''}
        onRequestPress={() =>
          selectedFilterOne === 'InProgress' &&
            selectedFilterTwo === 'PostedTrips' ?
            handleDeletePost(item) :
        ...
        packageName={item?.bookingTypePackage_name || item.booking_package_name || ''}
        postStatus={item?.post_status || ''}
